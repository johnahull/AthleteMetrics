-- Migration 0104: Add missing audit actions
-- Fixes: audit_logs constraint violation for organization_join_code_set and organization_type_accessed
-- These actions were used in code but not added to the audit_logs_action_valid constraint
...
BEGIN
  ...
  -- Recreate constraint with all actions including new ones
  ALTER TABLE audit_logs ADD CONSTRAINT audit_logs_action_valid CHECK (action IN (
    ...
    'organization_type_accessed',  -- NEW: org type configuration accessed
    'site_admin_access',
    ...
    'organization_join_code_set',  -- NEW: custom join code set by org admin
