...
-- 3. position requires sport when set (validation constraint enforced in application layer)
...
COMMENT ON INDEX idx_site_benchmarks_sport IS
  'Partial index for sport filtering - optimizes queries filtering by sport only. Excludes NULL values.';

COMMENT ON INDEX idx_custom_benchmarks_sport IS
  'Partial index for sport filtering - optimizes queries filtering by sport only. Excludes NULL values.';

COMMENT ON INDEX idx_site_benchmarks_sport_position IS
  'Composite index for sport+position filtering - optimizes queries filtering by both fields. Excludes NULL values.';
