-- Migration 0109: Add parent unlink audit action and resource type
-- Adds 'parent_unlinked_child' to audit_logs_action_valid constraint
-- Adds 'parent_athlete_link' to audit_logs_resource_type_valid constraint
--
-- Required for Phase 6: parent self-service unlink endpoint
-- (POST /api/parent/children/:athleteId/unlink)
-- IDEMPOTENT: Safe to run multiple times

DO $$
BEGIN
  -- ============================================================================
  -- 1. Extend audit_logs_action_valid with parent unlink action
  -- ============================================================================
  ...
  -- ============================================================================
  -- 2. Extend audit_logs_resource_type_valid with parent_athlete_link
  -- ============================================================================
  ...
  ALTER TABLE audit_logs ADD CONSTRAINT audit_logs_resource_type_valid CHECK (resource_type IN (
    ...
    'membership_request',
    'event',
    'parent_athlete_link'  -- NEW: parent-child link resource type
  ));

  RAISE NOTICE 'Migration 0109: Added parent_unlinked_child to action constraint and parent_athlete_link to resource_type constraint';
END $$;

COMMENT ON CONSTRAINT audit_logs_action_valid ON audit_logs IS
  'Valid audit log actions — added parent_unlinked_child for Phase 6 parent self-service unlink (migration 0109)';

COMMENT ON CONSTRAINT audit_logs_resource_type_valid ON audit_logs IS
  'Valid audit log resource types — added parent_athlete_link for Phase 6 parent self-service unlink (migration 0109)';
