...
PR preview environments create isolated deployments for each pull request, enabling code review and testing in a production-like environment. AthleteMetrics uses **GitHub Actions** to deploy and manage PR environments via the Railway CLI.

### How PR Environments Work

The deployment is handled by two GitHub Action workflows:

**Deployment (`.github/workflows/pr-deploy.yml`):**
1. **Developer/Claude Code creates a PR** → `gh pr create --base develop`
2. **GitHub Actions triggers** on PR open/sync/reopen
3. **Workflow builds** the application (type check + build)
4. **Railway CLI creates** environment `pr-{number}` if it doesn't exist
5. **Railway CLI deploys** code to the PR environment
6. **Health check** verifies deployment is ready
7. **GitHub Actions posts** preview URL comment on PR

**Cleanup (`.github/workflows/pr-cleanup.yml`):**
1. **PR is closed or merged**
2. **GitHub Actions triggers** cleanup workflow
3. **Railway CLI deletes** the `pr-{number}` environment
4. **PR comment updated** to indicate cleanup
...
| `RAILWAY_PROJECT_ID` | Railway project ID (for dashboard links in comments) |

These secrets are also used by `staging-deploy.yml` so they should already be configured.

### PR Environment Configuration

The `railway.json` file defines a `"pr"` environment template:
...
**Database:** PR environments should use the **testing database** (configure `DATABASE_URL` in Railway dashboard for PR environments) to keep staging data pristine.
...
**Note:** Custom domains are not automatically assigned to PR environments.

### PR Preview Comments

The `pr-deploy.yml` workflow automatically manages PR comments:
- Posts "deployment in progress" status when build starts
- Updates with success/failure when deployment completes
- Includes preview URL, Railway dashboard link, and troubleshooting tips
- Updates on PR sync to reflect new deployments
- Marks as "cleaned up" when PR is closed
