-- Migration 0107: Add training module audit log actions
-- Extends the audit_logs_action_valid constraint to include training module toggle events
...
BEGIN
  ...
  ALTER TABLE audit_logs ADD CONSTRAINT audit_logs_action_valid CHECK (action IN (
    ...
    -- Training module actions (added in migration 0107)
    'site_training_module_toggled',
    'org_training_enabled',
    'org_training_disabled'
  ));
END $$;

COMMENT ON CONSTRAINT audit_logs_action_valid ON audit_logs IS
  'Valid audit log actions - added site_training_module_toggled, org_training_enabled, org_training_disabled (training module feature) - migration 0107';
