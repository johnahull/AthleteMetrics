...
/**
 * Wellness access middleware - verifies user can access wellness features
 * Combines authentication check with wellness feature flag check
 * @param roleRequired - 'coach', 'org_admin', false (no role check), or undefined
 */
export const requireWellnessAccess = (roleRequired?: 'coach' | 'org_admin' | false) => {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const user = req.session?.user || req.user;
    ...
    // If we need to check role and have org ID (roleRequired is a string)
    if (roleRequired && typeof roleRequired === 'string' && organizationId) {
      const userRoles = await storage.getUserRoles(user.id, organizationId);
      if (!userRoles.includes(roleRequired) && !userRoles.includes('org_admin')) {
        return res.status(403).json({
          message: `${roleRequired} role required for wellness management`
        });
      }
    }
