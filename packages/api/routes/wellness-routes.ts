  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        // Determine if this is a future-scheduled request
        ...
        // Skip notifications for scheduled requests — the cron job will send them when due
        if (isScheduled) {
          res.status(201).json({
            ...request,
            scheduled: true,
        ...
        // Send notifications immediately for non-scheduled requests
        const notificationResults = await sendWellnessRequestNotifications(
          request,
          data,
          req.user!.id,
          organizationId
