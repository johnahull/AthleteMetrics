  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Dialog open={showAddDialog} onOpenChange={(open) => {
        ...
        if (!open) setSelectedBenchmarkIds(new Set());
      }}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
                ...
                {availableBenchmarks.map((benchmark) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        "w-full flex items-center gap-3 p-3 border rounded-lg cursor-pointer text-left transition-colors",
                        isSelected
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={selectedBenchmarkIds.size === 0 || addMutation.isPending}
            >
              {addMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Add{selectedBenchmarkIds.size > 0 ? ` (${selectedBenchmarkIds.size})` : " Selected"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
