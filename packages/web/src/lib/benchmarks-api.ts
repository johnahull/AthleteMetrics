...
// ============================================================================
// API Client Functions - Benchmark-based Filtering
// ============================================================================

/**
 * Fetch athletes filtered by benchmark achievement status
 * Returns array of athlete IDs that match the benchmark status criteria
 */
export async function fetchAthletesByBenchmarkStatus(
  organizationId: string,
  benchmarkId: string,
  ...
  const response = await fetch(
    `/api/analytics/benchmarks/${encodeURIComponent(benchmarkId)}/athletes?${params.toString()}`
  );
  if (!response.ok) {
    throw new Error('Failed to fetch athletes by benchmark status');
  }
  return response.json();
}

// ============================================================================
// React Query Hooks - Benchmark-based Filtering
// ============================================================================

/**
 * Hook to fetch athletes filtered by benchmark achievement status
 * Used for filtering analytics data by benchmark criteria
 */
export function useAthletesByBenchmarkStatus(
  organizationId: string,
  benchmarkId: string | undefined,
  status: 'met' | 'unmet' | 'no_data' | undefined,
  options?: {
    teamIds?: string[];
    genders?: string[];
    birthYearFrom?: number;
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchAthletesByBenchmarkStatus(
        organizationId,
        benchmarkId!,
        status!,
        options
      ),
    enabled: !!organizationId && !!benchmarkId && !!status,
    staleTime: STALE_TIME.REALTIME, // 1 minute (more frequent updates for filtering)
  });
}
